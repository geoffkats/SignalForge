"""Unit tests for predictor manifest loading and heuristic fallback."""
...
# ---------------------------------------------------------------------------
# SignalForgePredictor manifest loading
# ---------------------------------------------------------------------------
...
def test_predictor_loads_manifest_from_file(tmp_path):
    ...
    predictor = SignalForgePredictor(model_version="fallback", manifest_path=str(p))
...
def test_predictor_heuristic_predict_returns_correct_count():
    predictor = SignalForgePredictor(model_version="heuristic")
    genes = ["AR", "KLK3", "PTEN"]
